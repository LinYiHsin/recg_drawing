...
def cut_label_image(xyxy, image, label, path):
    import random
    import string

    dir_path = path.rsplit('.', 1)[0].lower()
    file_extension = path.rsplit('.', 1)[1].lower()
    Path(dir_path).mkdir(parents=True, exist_ok=True)
    # random_string = ''.join(random.choice(string.ascii_letters) for _ in range(5))
    # img_name = f'{ label }_{ random_string }.{ file_extension }'
    img_name = f'{ label }.{ file_extension }'
    ...
    w = x2 - x1
    aspext_ratio = w/h
    if aspext_ratio < (9/16) or aspext_ratio > (16/9):
    ...
    print('The label image saved in:{}\{}'.format(dir_path, img_name))
    cv2.imwrite('{}\{}'.format(dir_path, img_name), img)

    return

def plot_label_box(x, img, color=None, label=None, conf=None, line_thickness=None):
...
def detect(save_img=True):
    source, weights, view_img, save_txt, img_size, trace = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for index, item in enumerate(det_map):
                    ...
                    # Cut label images
                    cut_label_image(xyxy, im0_cpoy, label=f'{names[int(cls)]}_{index:02d}', path=save_path)

                    # Add bbox to image
                    plot_label_box(xyxy, im0, label=f'{names[int(cls)]}_{index:02d}', conf=conf, color=colors[int(cls)], line_thickness=2)
                    # plot_label_box(xyxy, im0, label=f'{names[int(cls)]}', conf=conf, color=colors[int(cls)], line_thickness=2)
