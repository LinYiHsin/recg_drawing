...
def cut_label_image(xyxy, image, label, index, path, rotate_result):
    from Tess_OCR import image_preprocessing

    if label == 'View':
        dir_path = path.rsplit('.', 1)[0].lower()
    else:
        dir_path = os.path.join(path.rsplit('.', 1)[0].lower(), label)
    file_extension = path.rsplit('.', 1)[1].lower()
    Path(dir_path).mkdir(parents=True, exist_ok=True)
    img_name = f'{ label }_{ index }.{ file_extension }'

    img = image.copy()
    x1, y1, x2, y2 = xyxy
    img = img[int(y1): int(y2), int(x1): int(x2)]
    if label=='DIM' and rotate_result:
        try:
            img = image_preprocessing(img)
        except:
            print(f'A problem occurred during {img_name} preprocessing.')
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for index, item in enumerate(det_map):
                    ...
                    # Cut label images
                    cut_label_image(xyxy, im0_cpoy, label=f'{names[int(cls)]}', index=f'{index:02d}', path=save_path, rotate_result=rotate)
